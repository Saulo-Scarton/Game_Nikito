	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end){
		...
		while(openList.size() > 0) {
			...
			if(current.tile.equals(end)) {
				//Chegou no ponto final, basta retornar o valor
			...
			for(int i = 0; i < 9; i++) {
				if(i == 4) continue; //posição 4 é onde o inimigo já está
				int x = current.tile.x;
				int y = current.tile.y;
				int xi = (i%3) - 1;
				int yi = (i/3) - 1;
				Tile tile = World.tiles[x+xi+((y+yi)*World.WIDTH)];//verificar se tile atual é parede ou caminho livre
